using Test
using SigmaProofs.Verificatum.Parser: Node, Leaf, Tree, encode, decode

function encoder_test(x)
    bytes = hex2bytes(replace(x, " " => ""))
    encoded_bytes = encode(decode(bytes))

    if encoded_bytes == bytes
        return true
    else
        println("Input: $x")
        println("encode(decode(x)): $(bytes2hex(encoded_bytes))")
        return false
    end
end
...
@test encoder_test("01 00000002 2D52")
@test encoder_test("00 00000002 01 00000001 AF 01 00000002 03E1")
@test encoder_test("00 00000002 00 00000002 01 00000001 AF 01 00000002 03E1 01 00000002 2D52")
